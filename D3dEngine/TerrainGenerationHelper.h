...
public:
	...
	std::shared_ptr<concurrency::concurrent_vector<std::shared_ptr<std::vector<std::vector<float>>>>> GetChunks() const { return m_chunks; };
	std::shared_ptr<concurrency::concurrent_vector<XMFLOAT4>> GetChunkOrigins() const { return m_chunkOrigins; };
...
private:
	...
	std::shared_ptr<concurrency::concurrent_vector<std::shared_ptr<std::vector<std::vector<float>>>>> m_chunks; /// chunks of the terrain
	std::shared_ptr<concurrency::concurrent_vector<XMFLOAT4>> m_chunkOrigins; /// chunk origins
